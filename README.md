...
赛兔（Sephiria）的独立锻体 Mod，使用游戏原生 AddOn 加载，当前源码版本 **2.8.5**。

消耗神器或石板，从三份配方中选择一份，获得本局永久属性；有色材料还可以给另一件神器增加附魔。配方会参考当前羁绊，提供构筑倾向与通用奖励。

## 下载安装

在 [Releases](https://github.com/Furxier/Sephiria-BodyForge/releases/latest) 下载已发布版本的 Mod ZIP 包（本次为 `赛兔锻体2.8.5（By残枫）… .zip`），解压后将 `BodyForge` 文件夹放入游戏的 `AddOns` 目录，然后启动游戏。
...
2.8.5 新增原版心之重担的装备成长：放在目标神器下方，按其羁绊选择 17 组装备属性之一，成长效果封顶 30 级。**此功能需要房主与客机均安装相同版本**；装备加成与吞噬的永久奖励独立。详细规则见包内使用说明。
...
16 组自动检查通过不等于长时间内存与全部分辨率的实机验收。原生永久属性对象和本地操作日志会随使用次数增长。反馈问题时请提供版本、单机／房主／客机、材料稀有度和所选配方。
